...
# Maximum seconds a single health check may run before being cancelled.
# A hung check (e.g. LLM health probe waiting for a TCP timeout) blocks the
# entire /health endpoint; this cap ensures the endpoint returns promptly.
...
HEALTH_ALLOWED_PATHS: frozenset[str] = frozenset({"/", "/health", "/health/detailed", "/health/startup", "/health/metrics", "/ready", "/version", "/metrics"})
