...
# Cumulative time budget (seconds) for retry sleeps shared across all
# concurrent _guarded_write operations.  Without a cap, N concurrent writes
# to a degraded filesystem each retry independently with 0.5 s → 1.0 s
# backoff, producing up to N × 1.5 s of aggregate sleep time that amplifies
# I/O pressure.  Once the shared budget is exhausted, subsequent retries
# skip sleeping and fail immediately, limiting total retry-induced delay.
DB_WRITE_RETRY_BUDGET_SECONDS: float = 3.0
...
# WAL synchronous mode: NORMAL is significantly faster than FULL and still
# safe with WAL journal mode.  Only the checkpoint thread needs FULL sync;
# normal writes are protected by the WAL.
SQLITE_SYNCHRONOUS: str = "NORMAL"

# Page cache size in KiB (negative value = KiB).  64 MB reduces disk reads
# for repeated queries on large databases (vector_memory, project_store).
SQLITE_CACHE_SIZE_KIB: int = -64000

# WAL auto-checkpoint threshold (pages).  Checkpoints run automatically
# after this many pages are written to the WAL.  1000 balances write
# throughput against recovery time.
SQLITE_WAL_AUTO_CHECKPOINT: int = 1000

# Busy timeout (milliseconds).  How long SQLite waits for a locked database
# before raising OperationalError.  5000 ms accommodates brief lock
