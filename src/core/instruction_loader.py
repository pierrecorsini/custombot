...
from src.constants import MAX_INSTRUCTION_FILE_SIZE
...
def _check_file_size(path: Path, max_bytes: int = MAX_INSTRUCTION_FILE_SIZE) -> None:
    """Reject instruction files exceeding the size cap."""
    size = path.stat().st_size
    if size > max_bytes:
        log.warning(
            "Instruction file exceeds size cap: %s (%d bytes > %d bytes)",
            path,
            size,
            max_bytes,
        )
        raise ValueError(
            f"Instruction file too large: {path} ({size} bytes > {max_bytes} bytes)"
        )
