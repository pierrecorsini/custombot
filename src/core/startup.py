...
class StartupContext(RegistryBackedMixin):
...
class _NoOpApplier:
    ...
    def apply(self, old_config: Any, new_config: Any) -> None:
...
async def _step_scheduler(ctx: StartupContext) -> str | None:
    ...
    scheduler = TaskScheduler(
        require_hmac=ctx.config.scheduler_require_hmac,
    )
...
async def _step_health_server(ctx: StartupContext) -> str | None:
    ...
    try:
        ...
        health_server = HealthServer(
            ...
            tool_executor=ctx.components.bot._tool_executor,
            app=ctx.app,
        )
...
class StartupOrchestrator(StepOrchestrator[StartupContext, ComponentSpec]):  # type: ignore[type-var]
