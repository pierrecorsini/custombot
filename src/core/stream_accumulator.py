...
class StreamAccumulator:
    ...
    _tool_calls_data: list[dict[str, Any]] = field(default_factory=list, init=False, repr=False)
    ...
    def build_completion(self) -> ChatCompletion:
        ...
        message = _Msg(
            ...
            tool_calls=tc_objects or None,  # type: ignore[arg-type]
            function_call=None,
        )
        choice = _Choice(
            index=0,
            message=message,
            finish_reason=self._finish_reason or "stop",  # type: ignore[arg-type]
        )
