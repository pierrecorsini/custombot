...
class MessageStore:
    ...
    def batch_message_exists(self, message_ids: list[str]) -> dict[str, bool]:
        """Batch-check which message IDs exist (single-pass dict lookup).

        Returns:
            Dict mapping each requested *message_id* to ``True`` (exists)
            or ``False`` (not found).
        """
        return {mid: (mid in self._message_id_index) for mid in message_ids}
