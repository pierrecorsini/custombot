...
- Pluggable ``connection_factory`` for sqlcipher3 or custom backends
...
Usage::
    ...
    # Or with a custom connection factory (e.g. sqlcipher3):
    # import sqlcipher3
    # pool = SqliteConnectionPool(connection_factory=sqlcipher3.connect)
...
from src.constants.db import (
    SQLITE_POOL_MAX_CONNECTIONS,
    SQLITE_POOL_MAX_IDLE_CONNECTIONS,
...
#
# WAL tuning rationale:
#   - journal_mode=WAL: concurrent readers + single writer, no read-blocking.
#   - synchronous=NORMAL: ~10× faster than FULL under WAL; still crash-safe
#     because the WAL itself is durable.  Only checkpoint needs FULL sync.
#   - cache_size=-64000: 64 MB page cache avoids repeated disk reads.
#   - wal_autocheckpoint=1000: auto-checkpoint after 1000 WAL pages written.
#   - busy_timeout=5000: wait up to 5 s for lock contention to resolve.
DEFAULT_PRAGMAS: list[str] = [
    "PRAGMA journal_mode=WAL;",
    "PRAGMA foreign_keys=ON;",
    f"PRAGMA synchronous={SQLITE_SYNCHRONOUS};",
    f"PRAGMA cache_size={SQLITE_CACHE_SIZE_KIB};",
    f"PRAGMA wal_autocheckpoint={SQLITE_WAL_AUTO_CHECKPOINT};",
    f"PRAGMA busy_timeout={SQLITE_BUSY_TIMEOUT_MS};",
]

# Type alias for a SQLite connection factory callable.
# Must accept the database path as the first positional argument and
# return a ``sqlite3.Connection`` (or compatible object like
# ``sqlcipher3.Connection``).  Keyword arguments such as
# ``check_same_thread`` are forwarded by the pool.
...
class SqliteConnectionPool:
    ...
    def __init__(
        ...
        *,
        ...
        self._connection_factory: ConnectionFactory = connection_factory or sqlite3.connect
    ...
    def get_connection(
        ...
        *,
        ...
        conn = self._connection_factory(str(path_unresolved), check_same_thread=False)
...
__all__ = ["SqliteConnectionPool", "ConnectionFactory", "DEFAULT_PRAGMAS"]
