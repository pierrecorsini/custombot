...
Batch dedup pre-filtering: before processing, all message IDs are
checked in a single batch via ``dedup.batch_check_inbound`` to skip
already-processed duplicates upfront, avoiding per-message async DB
lookups inside ``handle_message``.
"""
...
async def recover_pending_messages(
    ...
    When *dedup* is provided, all message IDs are pre-filtered via
    ``dedup.batch_check_inbound`` in chunks of ``DEDUP_BATCH_SIZE``,
    eliminating already-processed duplicates before they enter the
    per-message pipeline.  This reduces N individual async DB lookups
    to a single batch call per chunk.
    ...
    Args:
        ...
        dedup: Optional dedup service for batch pre-filtering of duplicates.
    ...
    # Batch dedup pre-filtering: check all message IDs in one pass.
    if dedup is not None:
        all_ids = [msg.message_id for msg in stale_messages if msg.message_id]
        if all_ids:
            dup_results = await dedup.batch_check_inbound(all_ids)
            before = len(stale_messages)
            stale_messages = [
                msg for msg in stale_messages
                if not dup_results.get(msg.message_id, False)
            ]
            skipped_dedup = before - len(stale_messages)
            if skipped_dedup:
                log.info(
                    "Batch dedup filtered %d/%d already-processed messages",
                    skipped_dedup,
                    before,
                )
