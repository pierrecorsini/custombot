...
from src.bot._event_helpers import _emit_event_safe
...
# Maximum number of tool calls executed concurrently via asyncio.gather.
MAX_PARALLEL_TOOL_CALLS: int = 5

# Default config: parallel tool execution enabled.
_PARALLEL_TOOL_EXECUTION: bool = True
...
class ReactIterationContext:
    ...
    # When True, independent tool calls run concurrently.
    ...
    for attempt in range(max_retries + 1):
        ...
        with llm_call_span(
            ...
            add_span_event(
                span,
                "llm_call_started",
                {"custombot.llm.iteration": iteration, "custombot.llm.streaming": use_streaming},
            )
            try:
                if use_streaming:
...
async def react_loop(
    ...
    Args:
        ...
        parallel_tool_execution: When True, independent tool calls run concurrently.
    ...
    * Returns a ``(text, tool_log, buffered_persist)`` tuple to terminate
    ...
    if has_tool_calls or finish_reason == "tool_calls":
        iteration_tool_log, iteration_buffered = await process_tool_calls(
            ...
            stream_callback=ctx.stream_callback,
            channel=ctx.channel,
            parallel=ctx.parallel_tool_execution,
        )
...
def _format_tool_summary(tool_log: list[ToolLogEntry]) -> str:
    """Format a summary of tools used during the ReAct loop."""
    if not tool_log:
        return ""
    unique_tools = dict.fromkeys(entry.name for entry in tool_log)
...
def format_max_iterations_message(iterations: int, tool_log: list[ToolLogEntry]) -> str:
    ...
    return (
        ...
        f"Try breaking it into smaller steps.{_format_tool_summary(tool_log)}"
    )
...
async def process_tool_calls(
    ...
    stream_callback: StreamCallback | None = None,
    channel: BaseChannel | None = None,
    parallel: bool = _PARALLEL_TOOL_EXECUTION,
) -> tuple[list[ToolLogEntry], list[dict[str, Any]]]:
    """Process tool calls from an LLM response and append results to messages.

    When *parallel* is True and tools are independent (no tool's output feeds
    another's input), executes all via ``asyncio.TaskGroup`` for concurrency.
    When dependencies are detected or *parallel* is False, falls back to
    ...
    with tool_calls_span(chat_id=chat_id, call_count=len(function_calls)):
        ...
        # Determine execution strategy: parallel when independent, sequential otherwise.
        use_parallel = (
            parallel
            and len(function_calls) > 1
            and _are_independent(function_calls)
            and len(function_calls) <= MAX_PARALLEL_TOOL_CALLS
        )

        results: list[tuple[str, str, ToolLogEntry]] = []
        if use_parallel:
            results = await _execute_parallel(function_calls, tool_executor, chat_id, workspace_dir, send_media)
        else:
            results = await _execute_sequential(function_calls, tool_executor, chat_id, workspace_dir, send_media)
...
def _are_independent(tool_calls: list[Any]) -> bool:
    """Heuristic: check whether tool calls have no inter-dependencies.

    Two tools are *dependent* when one tool's output could appear in
    another tool's arguments.  The heuristic checks for:
      - Duplicate tool names (same tool called twice may depend on order)
      - Any tool name appearing as a substring in another tool's arguments

    Note: O(n²) complexity but bounded by ``MAX_TOOL_CALLS_PER_TURN``
    (default 10), making it effectively O(1) in practice.

    Returns True when all tools appear independent (safe for parallel).
    """
    if len(tool_calls) <= 1:
        return True
    ...
    # Same tool called multiple times — may have ordering dependencies.
    if len(set(names)) != len(names):
        return False

    # Check if any tool name appears in another tool's arguments.
    for i, tc in enumerate(tool_calls):
        args_i = tc.function.arguments or "{}"
        for j, other_name in enumerate(names):
            if i != j and other_name in args_i:
                return False
...
) -> list[tuple[str, str, ToolLogEntry]]:
    """Execute tool calls concurrently via asyncio.TaskGroup."""
    results: list[tuple[str, str, ToolLogEntry]] = []
    try:
...
async def _execute_sequential(
    ...
    """Execute tool calls one at a time (sequential fallback)."""
    results: list[tuple[str, str, ToolLogEntry]] = []
    for tc in function_calls:
        result = await execute_tool_call(tool_executor, tc, chat_id, workspace_dir, send_media)
    ...
    with skill_execution_span(
        ...
        try:
            ...
            add_span_event(
                span,
                "tool_executed",
                {"custombot.skill.name": skill_name, "custombot.skill.result_length": len(result)},
            )
            return (
                tc_id,
                result,
