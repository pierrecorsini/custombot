...
**Serialization format**: each line is ``<CRC32>:<base64(msgpack)>``.
A CRC32 checksum guards against truncated writes and bit-rot from unclean
shutdowns.  Msgpack is ~3–5× faster than JSON for structured data with many
string fields, reducing persistence latency under burst traffic.  A JSON
fallback on read ensures backward compatibility with older queue files and
preserves crash-recovery readability (the JSON path is tried when msgpack
decoding fails).
...
def _encode_record(data: Dict[str, Any]) -> str:
    """Encode a dict to a CRC32-guarded base64-wrapped msgpack blob.

    Format: ``<8-hex-char CRC32>:<base64(msgpack(data))>``

    The CRC32 covers the raw msgpack bytes *before* base64 encoding,
    detecting truncated writes and bit-rot from unclean shutdowns.
    """
...
def _has_crc_prefix(line: str) -> bool:
    """Return True if *line* starts with the ``CRC32:`` guard prefix."""
    return (
        len(line) > 9
        and line[8] == ":"
        and all(c in "0123456789abcdefABCDEF" for c in line[:8])
    )


def _verify_crc32(line: str) -> bool:
    """Verify the CRC32 checksum embedded in a guarded line.

    Returns ``True`` when the stored CRC matches the computed one.
    Only call on lines where ``_has_crc_prefix()`` is ``True``.
    """
    crc_hex = line[:8]
    b64_part = line[9:]
    try:
        ...
        actual = binascii.crc32(raw) & 0xFFFFFFFF
...
def _decode_line(line: str, *, log_errors: bool = True) -> Optional[Dict[str, Any]]:
    ...
    Tries the current CRC-guarded msgpack+base64 format first, then
    falls back to unguarded msgpack+base64, and finally plain JSON so
    that queue files written by older versions (or manually inspected
    ...
    # Current format: CRC32:base64(msgpack)
    if _has_crc_prefix(stripped):
        if not _verify_crc32(stripped):
            if log_errors:
                log.warning("CRC32 checksum mismatch in queue line")
            return None
        try:
            raw = base64.b64decode(stripped[9:], validate=True)
    ...
    # Legacy path: unguarded msgpack + base64
    try:
...
class QueuePersistence:
    ...
    def _wal_append(self, lines: List[str]) -> None:
        ...
        # Step 1: Write to temp WAL file with flush+fsync for durability
        # before the atomic rename, so a crash mid-write leaves either the
        # complete file or nothing (no partial writes).
        with self._wal_tmp_file.open("w", encoding="utf-8") as f:
            f.write(content)
            f.flush()
            os.fsync(f.fileno())
    ...
    def load_pending(self) -> Tuple[Dict[str, QueuedMessage], Optional[QueueCorruptionResult]]:
        ...
        checksum_mismatch_lines: List[int] = []
        ...
        try:
            with self._queue_file.open("r", encoding="utf-8", errors="replace") as fh:
                for line_idx, line in enumerate(fh, start=1):
                    ...
                    # Pre-check CRC32 integrity for guarded lines
                    if _has_crc_prefix(line) and not _verify_crc32(line):
                        corrupted_count += 1
                        checksum_mismatch_lines.append(line_idx)
                        error_details.append(f"Line {line_idx}: CRC32 checksum mismatch")
                        continue
        ...
        corruption_result = QueueCorruptionResult(
            ...
            checksum_mismatches=checksum_mismatch_lines,
    ...
    def validate_sync(self) -> QueueCorruptionResult:
        ...
        Checks CRC32 checksums on guarded msgpack lines and validates
        all decodeable lines against the ``QueuedMessage`` schema.
        """
        from src.message_queue import MessageStatus, QueueCorruptionResult, QueuedMessage
        ...
        for line_num, line in enumerate(content.splitlines(), start=1):
            ...
            # Check CRC32 integrity for guarded lines
            if _has_crc_prefix(line) and not _verify_crc32(line):
                result.checksum_mismatches.append(line_num)
                result.error_details.append(f"Line {line_num}: CRC32 checksum mismatch")
                continue

            data = _decode_line(line, log_errors=True)
            if data is None:
                result.corrupted_lines.append(line_num)
                result.error_details.append(f"Line {line_num}: decode failed")
                continue
    ...
    def repair_sync(self, detection: QueueCorruptionResult) -> None:
        """Remove corrupted and checksum-mismatched lines from the queue file.

        Reads the file, filters out lines identified in the detection
        result (both corrupted and checksum-mismatched), and atomically
        rewrites with only valid content.
        """
        from src.utils.async_file import sync_atomic_write

        skip = set(detection.corrupted_lines) | set(detection.checksum_mismatches)
        if not skip:
            return
