...
class _TokenBucketRateLimiter:
    """Simple token-bucket rate limiter for LLM HTTP requests.

    Allows burst requests up to *burst* tokens, then enforces
    a steady *rate* (requests per second).  Thread-safe via
    a threading lock.
    """

    def __init__(self, rate: float = DEFAULT_LLM_RATE_LIMIT_RPS, burst: int = DEFAULT_LLM_RATE_LIMIT_BURST) -> None:
        self._rate = rate
        self._burst = burst
...
class _PerMinuteRateLimiter:
    """Sliding-window rate limiter enforcing requests and tokens per minute.

    Tracks API call count and cumulative token usage within a 60-second
    sliding window.  When either limit is exceeded, the caller must wait.
    ...
    def __init__(
        ...
        self._lock = threading.Lock()
        # (monotonic_timestamp, token_count) pairs — deque for O(1) popleft
    ...
    async def acquire(self, estimated_tokens: int = 0) -> None:
        """Wait until the request is within rate limits.

        Args:
            estimated_tokens: Expected token usage for this request,
                used to pre-check the token budget.
        """
        while True:
            with self._lock:
                now = time.monotonic()
                ...
                token_count = sum(t for _, t in self._entries)
                if (
                    request_count < self._max_requests
                    and token_count + estimated_tokens <= self._max_tokens
                ):
                    self._entries.append((now, 0))
                    return
            await asyncio.sleep(0.1)

    def record_tokens(self, count: int) -> None:
        """Record actual token usage from the last API response.

        Updates the most recent entry with the real token count.
        """
        with self._lock:
            if self._entries:
                ts, _ = self._entries[-1]
                self._entries[-1] = (ts, count)
...
class LLMClient:
    def __init__(
        ...
        self._per_minute_limiter = _PerMinuteRateLimiter(
            max_requests_per_minute=getattr(cfg, "max_requests_per_minute", 60),
            max_tokens_per_minute=getattr(cfg, "max_tokens_per_minute", 100_000),
        )
    ...
    async def _health_probe_loop(self) -> None:
        ...
        Two-stage probe:
          1. Lightweight ``models.list()`` — tests auth and network.
          2. Minimal completion (``"hi"`` with ``max_tokens=1``) — tests the
             full inference pipeline.  Only attempted when the models probe
             fails, to distinguish auth/network issues from capacity issues.
        """
        try:
            while self._circuit_breaker.state == CircuitState.OPEN:
                ...
                try:
                    ...
                    pass
                # Stage 2: minimal completion probe (exercises full pipeline)
                try:
                    ...
                    await self._circuit_breaker.force_close()
                    log.info("LLM completion probe succeeded — circuit breaker force-closed")
                    return
    ...
    async def chat(
        ...
        """
        # Rate limiter: throttle HTTP requests before they reach the provider
        await self._rate_limiter.acquire()
        await self._per_minute_limiter.acquire()
        ...
        async with self._circuit_protected_call("LLM"):
            ...
            # Record actual token usage for per-minute rate limiting
            if result.usage:
                total = (result.usage.total_tokens
                         or (result.usage.prompt_tokens or 0) + (result.usage.completion_tokens or 0))
                if total > 0:
                    self._per_minute_limiter.record_tokens(total)
            return result
    ...
    async def chat_stream(
        ...
        try:
            async with self._circuit_protected_call("LLM streaming"):
                ...
                self._record_stream_tokens(acc.usage_data)
    ...
    def _record_stream_tokens(self, usage_data: Any) -> None:
        """Record stream token usage in the per-minute rate limiter."""
        if not usage_data:
            return
        if isinstance(usage_data, dict):
            ...
            prompt = usage_data.prompt_tokens or 0
            completion = usage_data.completion_tokens or 0
    ...
    def http_client(self) -> httpx.AsyncClient:
        """Underlying httpx connection pool for TCP connection reuse.

        Allows sibling ``AsyncOpenAI`` instances (e.g. for embeddings)
        to share the same connection pool when targeting the same host,
        avoiding duplicate TCP connections.
        """
