...
# Rough per-1K-token USD estimates (GPT-4o-class pricing, May 2025).
# Conservative defaults; actual model pricing may differ.
DEFAULT_COST_PER_1K_PROMPT_TOKENS: float = 0.005
DEFAULT_COST_PER_1K_COMPLETION_TOKENS: float = 0.015
...
class TokenUsage:
    ...
    def estimate_cost_usd(
        self,
        *,
        cost_per_1k_prompt: float = DEFAULT_COST_PER_1K_PROMPT_TOKENS,
        cost_per_1k_completion: float = DEFAULT_COST_PER_1K_COMPLETION_TOKENS,
    ) -> float:
        """Estimate total USD cost based on accumulated token counts.

        Uses per-1K-token pricing defaults (GPT-4o-class).  Callers
        can override with model-specific rates.
        """
        with self._lock:
            prompt_cost = (self.prompt_tokens / 1000.0) * cost_per_1k_prompt
            completion_cost = (self.completion_tokens / 1000.0) * cost_per_1k_completion
...
def _make_per_chat_map(max_size: int = DEFAULT_PER_CHAT_TOKEN_TRACKING_SIZE) -> dict[str, dict[str, int]]:
    ...
    try:
        ...
        return BoundedOrderedDict(max_size=max_size, eviction="half")  # type: ignore[return-value]
    except ImportError:
        return {}
