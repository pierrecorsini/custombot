...
from src.security.signing import IntegrityError, get_scheduler_secret, read_signature_file, sign_payload, verify_payload
...
class TaskScheduler(BaseBackgroundService):
    ...
    def __init__(self, *, require_hmac: bool = True) -> None:
        super().__init__()
        ...
        # Whether HMAC is mandatory for loading tasks
        self._require_hmac: bool = require_hmac
        # Cached UTC offset
        ...
        # Unified dedup service — defaults to NullDedupService so callers
        # never need None-checks; replaced via set_dedup_service() at startup.
        self._dedup: DeduplicationService = NullDedupService()
    ...
    async def _load(self, chat_id: str) -> None:
        ...
        HMAC verification is mandatory when ``scheduler_require_hmac`` is True.
        If the HMAC secret is not configured and mandatory mode is enabled,
        tasks are refused.  If the secret IS configured but verification fails,
        ...
        try:
            ...
            if raw is not None:
                ...
                if secret is not None:
                    # Secret configured — verify signature.
                    ...
                    if stored_sig is None:
                        ...
                        raise IntegrityError(
                            f"HMAC signature file missing for {chat_id}"
                        )
                    if not verify_payload(secret, raw.encode("utf-8"), stored_sig):
                        ...
                        raise IntegrityError(
                            f"HMAC signature mismatch for {chat_id}"
                        )
                else:
                    # No secret configured — refuse if mandatory.
                    if self._require_hmac:
                        log.error(
                            "Scheduler HMAC is mandatory (scheduler_require_hmac=True) "
                            "but SCHEDULER_HMAC_SECRET is not set — refusing to load "
                            "tasks for %s",
                            chat_id,
                        )
                        log.warning(
                            "AUDIT: scheduler_integrity_fail chat_id=%s "
                            "reason=hmac_mandatory_but_no_secret",
                            chat_id,
                        )
                        return
            ...
            # Re-raise integrity errors — do not silently swallow.
            raise
    ...
    async def load_all(self) -> None:
        ...
        Logs a startup warning if HMAC is mandatory but not configured.
        """
        if not self._workspace or not self._workspace.exists():
            return

        # Startup warning when HMAC is mandatory but no secret configured.
        secret = get_scheduler_secret()
        if self._require_hmac and secret is None:
            log.warning(
                "SECURITY: scheduler_require_hmac is True but "
                "SCHEDULER_HMAC_SECRET is not set — scheduled tasks "
                "will be refused. Set the SCHEDULER_HMAC_SECRET "
                "environment variable or set scheduler_require_hmac=False "
                "in config.json."
            )
        ...
        # Load all chat tasks concurrently.  IntegrityError from one chat
        # must not prevent other chats from loading.
        results = await asyncio.gather(
            *(self._load(chat_id) for chat_id in valid_dirs),
            return_exceptions=True,
        )
        for chat_id, result in zip(valid_dirs, results):
            if isinstance(result, IntegrityError):
                log.error(
                    "Refused to load tasks for %s: HMAC integrity check failed",
                    chat_id,
                )
    ...
    async def _execute_task(self, chat_id: str, task: dict[str, Any]) -> None:
        ...
        try:
            ...
            if not result:
                ...
                if self._dedup.check_and_record_outbound(chat_id, formatted):
                    log.info(
        ...
        try:
            while self._running:
                ...
                try:
                    ...
                    log.critical(
                        "Scheduler loop received non-recoverable signal (%s): %s",
                        type(exc).__name__,
                        exc,
                        exc_info=True,
                    )
