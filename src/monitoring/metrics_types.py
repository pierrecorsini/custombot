...
@dataclass(slots=True)
class ChatReactIterations:
    """Per-chat ReAct loop iteration count entry for distribution tracking."""

    chat_id: str
    iteration_count: int

    def to_dict(self) -> dict[str, Any]:
        return {"chat_id": self.chat_id, "iteration_count": self.iteration_count}


@dataclass(slots=True)
class ChatLatencyPercentiles:
    """Per-chat message processing latency percentiles."""
...
@dataclass(slots=True)
class DedupSnapshotStats:
    """Dedup stats including buffer metrics for periodic logging."""
    ...
    @property
    def inbound_hit_rate(self) -> float:
        total = self.inbound_hits + self.inbound_misses
        return self.inbound_hits / total if total else 0.0

    @property
    def outbound_hit_rate(self) -> float:
        total = self.outbound_hits + self.outbound_misses
        return self.outbound_hits / total if total else 0.0

    def to_dict(self) -> dict[str, Any]:
        return {
            "inbound_hits": self.inbound_hits,
            "inbound_misses": self.inbound_misses,
            "inbound_hit_rate": round(self.inbound_hit_rate, 4),
            "outbound_hits": self.outbound_hits,
            "outbound_misses": self.outbound_misses,
            "outbound_hit_rate": round(self.outbound_hit_rate, 4),
...
class PerformanceSnapshot:
    ...
    top_chat_latencies: list[ChatLatencyPercentiles] = field(default_factory=list)
    top_chat_react_iterations: list[ChatReactIterations] = field(default_factory=list)
    ...
    # Routing match cache effectiveness
    routing_cache_hits: int = 0
    routing_cache_misses: int = 0

    # Skill registry lookup cache effectiveness
    skill_registry_cache_hits: int = 0
    skill_registry_cache_misses: int = 0
    ...
    # Error-rate alert thresholds currently in effect (window_seconds → errors/min)
    error_alert_thresholds: dict[int, float] = field(default_factory=dict)

    # Message rejection counts by reason label (acl_rejected, empty_text, etc.)
    message_rejection_counts: dict[str, int] = field(default_factory=dict)

    # Scheduled task metrics (separate from interactive message processing)
    scheduled_task_count: int = 0
    scheduled_task_latency: LatencyStats = field(default_factory=LatencyStats)

    # DB retry budget metrics
    retry_budget_ratio: float = 1.0
    retry_budget_recovery_eta_seconds: float = 0.0

    # Conversation quality metrics (from quality_metrics module)
    quality_metrics: dict[str, Any] = field(default_factory=dict)

    # LLM latency anomaly detection (from anomaly_detector module)
    anomaly_stats: dict[str, Any] = field(default_factory=dict)

    # Routing engine latency metrics
    routing_latency: LatencyStats = field(default_factory=LatencyStats)
    routing_match_count: int = 0
    routing_miss_count: int = 0

    # Per-skill sliding-window error rates (skill_name → rate 0.0–1.0)
    skill_error_rates: dict[str, float] = field(default_factory=dict)
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "database": {
                ...
                "retry_budget_ratio": round(self.retry_budget_ratio, 4),
                "retry_budget_recovery_eta_seconds": round(
                    self.retry_budget_recovery_eta_seconds, 2
                ),
            },
            "react_iterations": {
            ...
            "top_chat_latencies": [l.to_dict() for l in self.top_chat_latencies],
            "top_chat_react_iterations": [r.to_dict() for r in self.top_chat_react_iterations],
            "memory_cache": {
            ...
            "routing_cache": {
                "hits": self.routing_cache_hits,
                "misses": self.routing_cache_misses,
                "hit_ratio": round(
                    self.routing_cache_hits / (self.routing_cache_hits + self.routing_cache_misses), 4
                )
                if (self.routing_cache_hits + self.routing_cache_misses) > 0
                else 0.0,
            },
            "skill_registry_cache": {
                "hits": self.skill_registry_cache_hits,
                "misses": self.skill_registry_cache_misses,
                "hit_ratio": round(
                    self.skill_registry_cache_hits
                    / (self.skill_registry_cache_hits + self.skill_registry_cache_misses),
                    4,
                )
                if (self.skill_registry_cache_hits + self.skill_registry_cache_misses) > 0
                else 0.0,
            },
            ...
            "message_rejections": dict(self.message_rejection_counts),
            "scheduled_tasks": {
                "count": self.scheduled_task_count,
                "latency": self.scheduled_task_latency.to_dict(),
            },
            "routing": {
                "latency": self.routing_latency.to_dict(),
                "match_count": self.routing_match_count,
                "miss_count": self.routing_miss_count,
                "match_rate": round(
                    self.routing_match_count
                    / (self.routing_match_count + self.routing_miss_count),
                    4,
                )
                if (self.routing_match_count + self.routing_miss_count) > 0
                else 0.0,
            },
            "skill_error_rates": dict(self.skill_error_rates),
            "error_rates": {
                ...
                "alert_thresholds": {
                    f"{k // 60}m": v for k, v in self.error_alert_thresholds.items()
                },
            },
