...
class LRULockCache:
    ...
    When ``ttl`` is set, idle locks not accessed within ``ttl`` seconds are
    lazily evicted on the next access, reclaiming memory from transient group
    chats whose locks would otherwise persist indefinitely.
    ...
    Attributes:
        ...
        _ttl: Optional time-to-live in seconds; None disables TTL eviction
        _timestamps: Dict tracking last-access time per key (for TTL)
    ...
    def __init__(
        ...
        ttl: float | None = DEFAULT_LOCK_CACHE_TTL,
    ) -> None:
        """
        ...
        Args:
            ...
            ttl: Optional time-to-live in seconds.  When set, locks not accessed
                within this window are treated as expired and eligible for
                eviction.  Accessing an expired key transparently creates a new
                lock.  None (default) disables TTL-based eviction.
        """
    ...
    def _is_alive(self, key: str) -> bool:
        ...
        if self._ttl is None:
            return True
        return (time.monotonic() - self._timestamps.get(key, 0.0)) < self._ttl
    ...
    async def get_or_create(self, key: str) -> asyncio.Lock:
        ...
        If the key exists and is within its TTL window, moves it to the end
        (most recently used) and refreshes its timestamp.  If the key exists
        but has expired, the stale entry is removed and a new lock is created.
    ...
    def _evict_one(self) -> None:
        ...
        When TTL is configured, expired unreferenced entries are evicted first
        to reclaim idle locks from transient group chats before falling back
        to standard LRU eviction.
        """
        # Eager TTL cleanup: evict any expired, unreferenced, unlocked entries
        if self._ttl is not None:
            now = time.monotonic()
            for k in list(self._cache.keys()):
                if (
                    self._ref_counts.get(k, 0) == 0
                    and not self._cache[k].locked()
                    and (now - self._timestamps.get(k, 0.0)) >= self._ttl
                ):
                    self._cache.pop(k)
    ...
    def stats(self) -> dict[str, int | float]:
        ...
        ``max_chat_lock_cache_size`` and ``max_chat_lock_cache_ttl``.
        """
...
class LRUDict:
    ...
    def items(self) -> Any:
        """Return an iterator over (key, value) pairs (no LRU promotion)."""
        return self._cache.items()

    def keys(self) -> Any:
        """Return an iterator over keys (no LRU promotion)."""
        return self._cache.keys()

    def values(self) -> Any:
        """Return an iterator over values (no LRU promotion)."""
