...
from src.constants.network import (
    DEFAULT_EMBEDDING_HTTPX_MAX_CONNECTIONS,
    DEFAULT_EMBEDDING_HTTPX_MAX_KEEPALIVE_CONNECTIONS,
)
...
CONFIG_SCHEMA: dict[str, Any] = {
    ...
    "properties": {
        ...
        "llm": {
            ...
            "properties": {
                ...
                "embedding_max_connections": {
                    ...
                    "minimum": 1,
                    "maximum": 100,
                    "description": "Max concurrent TCP connections for the dedicated embedding HTTP client (default: 8)",
                },
                "embedding_max_keepalive_connections": {
                    "type": "integer",
                    "minimum": 1,
                    "maximum": 100,
                    "description": "Max idle keep-alive connections for the dedicated embedding HTTP client (default: 4)",
                },
                "stream_response": {
                    "type": "boolean",
        ...
        "max_chat_lock_cache_ttl": {
            "type": ["number", "null"],
            "minimum": 0,
            "description": "Time-to-live (seconds) for per-chat lock cache entries. Idle locks not accessed within this window are lazily evicted, reclaiming memory from transient group chats. null (default) disables TTL-based eviction. Recommended: 3600 (1 hour).",
        },
        "shell": {
            "type": "object",
        ...
        "middleware": {
            ...
            "properties": {
                ...
                "middleware_priorities": {
                    ...
                    "description": "Priority overrides: middleware class name → priority int (lower runs first)",
                },
            },
            "additionalProperties": False,
        },
        ...
        "scheduler_require_hmac": {
            "type": "boolean",
            "description": "When True, scheduler tasks will NOT be loaded unless HMAC is configured. Prevents unauthorized task injection. Default: True.",
        },
        "skills_version_check": {
            "type": "boolean",
            "description": "When True, verify skill version compatibility on load. Rejects skills with unmet version-range dependencies. Default: True.",
        },
        "redis_url": {
            "type": "string",
            "description": "Redis URL for optional caching layer (e.g. 'redis://localhost:6379'). Empty string (default) disables Redis.",
        },
        "redis_enabled": {
            "type": "boolean",
            "description": "Enable Redis as a caching backend for hot data (chat contexts, routing rules, dedup state). Falls back to in-memory cache when disabled or unavailable. Default: False.",
        },
        "lazy_load_skills": {
            "type": "boolean",
            "description": "Defer importing skill modules until first use instead of importing all at startup. Core skills (think, etc.) are always eagerly loaded. Default: True.",
        },
    },
...
class LLMConfig:
    ...
    # Connection pool limits for the dedicated embedding httpx client.
    # Only used when embedding_base_url is set (dedicated client path).
    # When sharing the LLM client, the LLM pool limits apply.
    embedding_max_connections: int = DEFAULT_EMBEDDING_HTTPX_MAX_CONNECTIONS
    embedding_max_keepalive_connections: int = DEFAULT_EMBEDDING_HTTPX_MAX_KEEPALIVE_CONNECTIONS
...
class MiddlewareConfig:
    ...
    Priority ordering:
        ...
        Override priorities via ``middleware_priorities`` dict mapping
        middleware class names (e.g. ``"ErrorHandlerMiddleware"``) to
        priority integers.  When set, middleware are sorted by priority
        after construction, overriding the list order.
    """
    ...
    # Priority overrides: class name → priority int.  Lower runs first.
    # Example: {"ErrorHandlerMiddleware": 1000, "MetricsMiddleware": 50}
    middleware_priorities: Dict[str, int] = field(default_factory=dict)

    def __repr__(self) -> str:
        return (
            f"MiddlewareConfig(order={self.middleware_order or 'default'}, "
            f"extra={len(self.extra_middleware_paths)}, "
            f"priorities={self.middleware_priorities or 'default'})"
        )
...
class Config:
    ...
    # TTL (seconds) for per-chat lock cache entries.  Idle locks not accessed
    # within this window are lazily evicted, reclaiming memory from transient
    # group chats.  None disables TTL-based eviction (default, backward compat).
    max_chat_lock_cache_ttl: Optional[float] = DEFAULT_LOCK_CACHE_TTL
    ...
    # When True, scheduler tasks will NOT be loaded unless an HMAC secret is
    # configured via the SCHEDULER_HMAC_SECRET environment variable.  Prevents
    # unauthorized task injection in production deployments.  Default True for
    # new installations; set to False for legacy deployments without HMAC.
    scheduler_require_hmac: bool = True
    # When True, verify skill version compatibility on load.  Skills with
    # unmet version-range dependencies are rejected.  Default True.
    skills_version_check: bool = True
    # Redis caching backend configuration.
    # When enabled and redis_url is set, hot data (chat contexts, routing rules,
    # dedup state) is cached in Redis with automatic TTL management.  Falls back
    # to an in-memory dict cache when Redis is unavailable or the ``redis``
    # package is not installed.
    redis_url: str = ""
    redis_enabled: bool = False
    # When True, skill modules are imported on first use instead of at startup,
    # reducing cold-start latency.  Core skills are always eagerly loaded.
...
def _validate_string_constraints(value: str, schema: dict[str, Any], path: str) -> List[ValidationError]:
...
def _validate_against_schema(
    data: Any, schema: dict[str, Any], path: List[str | int], errors: List[ValidationError]
) -> None:
