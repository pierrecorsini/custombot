...
from typing import Any, Dict, List, Set, Tuple, get_type_hints, TYPE_CHECKING, cast
...
def _collect_known_field_names(cls: type[Any]) -> Dict[str, List[str]]:
    ...
    def _collect(dc: type[Any], path: str = "") -> None:
        try:
            hints = get_type_hints(dc)
        except Exception:
            log_noncritical(
                NonCriticalCategory.TYPE_RESOLUTION,
                f"Failed to resolve type hints for {getattr(dc, '__name__', dc)}, falling back to empty hints",
                logger=log,
            )
            hints = {}
...
def _check_unknown_keys(data: dict[str, Any], file_path: Path) -> None:
    ...
    def _check(obj: dict[str, Any], parent: str = "") -> None:
...
def _redact_secrets(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return cast(Dict[str, Any], redact_recursive(data))
...
def _log_validation_errors(errors: List[ValidationError], file_path: Path) -> None:
