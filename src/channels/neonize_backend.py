...
# Reconnect backoff constants (exponential backoff with full jitter).
...
class NeonizeBackend:
    ...
    def __init__(self, cfg: WhatsAppConfig) -> None:
        ...
        # Reconnection backoff state (exponential with full jitter)
        self._reconnect_delay: float = RECONNECT_INITIAL_DELAY
    ...
    async def _watchdog(self, gen: int) -> None:
        ...
        while self._watchdog_gen == gen and self._client is not None:
            ...
            try:
                ...
                cap = min(RECONNECT_MAX_DELAY, RECONNECT_INITIAL_DELAY * (RECONNECT_MULTIPLIER ** self._reconnect_attempts))
                delay_with_jitter = random.uniform(0, cap)
                log.warning(
                    "Watchdog reconnect failed (attempt %d): %s — next in %.1fs",
                    self._reconnect_attempts,
                    exc,
                    delay_with_jitter,
                )
                await asyncio.sleep(delay_with_jitter)
                self._reconnect_delay = min(cap, RECONNECT_MAX_DELAY)
