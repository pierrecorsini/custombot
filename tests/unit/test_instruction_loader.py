...
Covers mtime caching, frontmatter stripping, file I/O, cache invalidation,
and file-size cap enforcement.
"""
...
from src.core.instruction_loader import InstructionLoader, _check_file_size
...
class TestFileSizeCap:
    def test_load_rejects_oversized_file(self, loader: InstructionLoader, instructions_dir: Path):
        big = instructions_dir / "big.md"
        big.write_text("x" * 100, encoding="utf-8")
        with pytest.raises(ValueError, match="too large"):
            _check_file_size(big, max_bytes=50)

    def test_load_raw_rejects_oversized_file(
        self, loader: InstructionLoader, instructions_dir: Path
    ):
        big = instructions_dir / "big.md"
        big.write_text("x" * 100, encoding="utf-8")
        with patch(
            "src.core.instruction_loader._check_file_size",
            side_effect=ValueError("too large"),
        ):
            with pytest.raises(ValueError, match="too large"):
                loader.load_raw("big.md")

    def test_load_allows_file_within_cap(self, loader: InstructionLoader, instructions_dir: Path):
        _write_instruction(instructions_dir / "ok.md", "small content")
        result = loader.load("ok.md")
        assert result == "small content"

    def test_load_raw_allows_file_within_cap(self, loader: InstructionLoader, instructions_dir: Path):
        _write_instruction(instructions_dir / "ok.md", "small content")
        result = loader.load_raw("ok.md")
        assert result == "small content"
