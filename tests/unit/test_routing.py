...
class TestRoutingMatchCache:
    ...
    # ── Cache hit/miss ratio assertions ──────────────────────────────────

    @staticmethod
    def _two_rule_engine(instruction: str = "matched.md") -> RoutingEngine:
        """Build an engine with 2 rules so the single-rule fast-path is bypassed.

        The _single_rule optimisation skips the cache entirely when only one
        rule is loaded.  Using two rules forces _match_impl through the
        cache-key / lookup / store path.
        """
        engine = RoutingEngine(Path("/dummy"))
        engine._rules = [
            make_rule(id="r1", instruction=instruction),
            make_rule(id="r2", sender="never-match", instruction="fallback.md"),
        ]
        return engine

    async def test_cache_hit_ratio_repeated_same_message(self):
        """Repeated matches of the same message achieve 100% cache hit rate.

        After the initial miss/populate, every subsequent call should hit
        the cache — verified by checking cache size stays at 1.
        """
        engine = self._two_rule_engine("ratio.md")
        msg = make_msg(text="hit-ratio-test")

        # First call: cache miss (populates the entry)
        await engine.match(msg)
        assert len(engine._match_cache) == 1

        # Subsequent calls: all cache hits — cache size must not grow
        for _ in range(50):
            await engine.match(msg)
        assert len(engine._match_cache) == 1, "All subsequent calls must be cache hits"

    async def test_cache_hit_ratio_mixed_workload(self):
        """Mixed workload: unique messages miss, repeats hit the cache.

        Sequence of 5 unique messages followed by 5 repeats of the first
        message should produce exactly 5 cache entries (no growth on repeats).
        """
        engine = self._two_rule_engine("mixed.md")
        unique_messages = [make_msg(sender_id=f"u{i}", text=f"text-{i}") for i in range(5)]

        # Populate 5 distinct cache entries
        for m in unique_messages:
            await engine.match(m)
        assert len(engine._match_cache) == 5

        # Repeat the first message 5 times — all hits, no new entries
        for _ in range(5):
            await engine.match(unique_messages[0])
        assert len(engine._match_cache) == 5, "Repeated messages must be cache hits"

    async def test_cache_eviction_preserves_recently_accessed(self):
        """After LRU eviction, recently accessed entries survive while old ones are evicted.

        With a small cache, fill it to capacity, then access entry-0 to make it
        MRU, overflow the cache, and verify entry-0 is still present (it was at
        the MRU end and survives the half-eviction of oldest entries).
        """
        from src.utils import BoundedOrderedDict

        engine = self._two_rule_engine("evict-recent.md")

        # Use a small cache to make eviction behaviour deterministic
        small_max = 10
        engine._match_cache = BoundedOrderedDict(
            max_size=small_max, eviction="half", ttl=60.0
        )

        # Access msg_hot first (becomes oldest in insertion order)
        msg_hot = make_msg(sender_id="hot", text="keep-me")
        await engine.match(msg_hot)
        assert len(engine._match_cache) == 1

        # Fill the rest of the cache with unique entries (after hot)
        for i in range(1, small_max):
            await engine.match(make_msg(sender_id=f"fill-{i}", text=f"msg-{i}"))
        assert len(engine._match_cache) == small_max

        # Now re-access msg_hot to move it to the MRU end (most recently used)
        await engine.match(msg_hot)

        # Overflow by 1 to trigger half-eviction (oldest half removed)
        await engine.match(make_msg(sender_id="overflow", text="over"))
        assert len(engine._match_cache) <= small_max

        # msg_hot was re-accessed (MRU) so it should have survived eviction.
        # Verify by checking the cache doesn't grow on a re-match.
        size_before = len(engine._match_cache)
        await engine.match(msg_hot)
        assert len(engine._match_cache) == size_before, "Hot entry should still be cached"

    async def test_cache_ttl_expiry_all_entries(self):
        """When all entries expire simultaneously, the cache behaves as empty (all misses).

        BoundedOrderedDict lazily removes expired entries on get(), so __len__
        may still count stale entries until they are accessed. We verify that
        expired entries produce fresh results (misses) by checking timestamps.
        """
        engine = self._two_rule_engine("ttl-expire.md")

        # Populate several entries
        msgs = [make_msg(sender_id=f"s{i}", text=f"t{i}") for i in range(10)]
        for m in msgs:
            await engine.match(m)
        assert len(engine._match_cache) == 10

        # Record timestamps before expiry
        timestamps_before = {}
        for m in msgs:
            key = engine._cache_key(MatchingContext.from_message(m))
            _, ts = engine._match_cache._cache[key]
            timestamps_before[key] = ts

        # Age all entries past TTL
        stale_time = time.monotonic() - 100.0
        for key in list(engine._match_cache._cache.keys()):
            value, _ = engine._match_cache._cache[key]
            engine._match_cache._cache[key] = (value, stale_time)

        # Re-access first 3 messages — should be cache misses that repopulate
        for m in msgs[:3]:
            await engine.match(m)

        # Verify the 3 re-accessed messages got fresh timestamps (cache miss → repopulate)
        for m in msgs[:3]:
            key = engine._cache_key(MatchingContext.from_message(m))
            _, new_ts = engine._match_cache._cache[key]
            assert new_ts > stale_time, f"Re-accessed entry should have fresh timestamp"
            assert new_ts != timestamps_before[key], "Timestamp should differ from original"

    async def test_cache_hit_ratio_with_partial_ttl_expiry(self):
        """Partial TTL expiry: expired entries miss, fresh entries still hit.

        Populate cache, expire half the entries, then verify:
        - Fresh entries still produce cache hits (no cache growth)
        - Expired entries produce misses (cache repopulates with fresh timestamp)
        """
        engine = self._two_rule_engine("partial-ttl.md")

        fresh_msgs = [make_msg(sender_id=f"fresh{i}", text=f"fresh{i}") for i in range(5)]
        stale_msgs = [make_msg(sender_id=f"stale{i}", text=f"stale{i}") for i in range(5)]

        # Populate all 10 entries
        for m in fresh_msgs + stale_msgs:
            await engine.match(m)
        assert len(engine._match_cache) == 10

        # Expire only the "stale" entries
        for m in stale_msgs:
            key = engine._cache_key(MatchingContext.from_message(m))
            value, _ = engine._match_cache._cache[key]
            engine._match_cache._cache[key] = (value, time.monotonic() - 100.0)

        # Re-access fresh messages — all should be cache hits
        for m in fresh_msgs:
            await engine.match(m)
        # Stale entries may have been lazily cleaned during get(); no new entries added
        assert len(engine._match_cache) <= 10, "Fresh entries hit; no new entries added"

        # Re-access stale messages — should be cache misses that repopulate
        for m in stale_msgs:
            await engine.match(m)
        # All 10 entries should now be fresh again
        assert len(engine._match_cache) == 10

    async def test_cache_no_match_hit_ratio(self):
        """Cache hit ratio is maintained even for no-match (None) results.

        When no rules match, (None, None) is cached. Subsequent identical
        messages should still hit the cache without re-evaluating rules.
        """
        engine = RoutingEngine(Path("/dummy"))
        # Two rules that both exclude the test sender → no match, cached as (None, None)
        engine._rules = [
            make_rule(id="r1", sender="nonexistent-a"),
            make_rule(id="r2", sender="nonexistent-b"),
        ]
        msg = make_msg(sender_id="wont-match")

        # First call: miss → caches (None, None)
        result1 = await engine.match(msg)
        assert result1 is None
        assert len(engine._match_cache) == 1

        # Subsequent calls: all cache hits
        for _ in range(20):
            result = await engine.match(msg)
            assert result is None
        assert len(engine._match_cache) == 1, "All None-result calls must be cache hits"

    async def test_cache_eviction_ratio_at_capacity(self):
        """After filling to max capacity, the cache should not exceed max_size.

        Verifies that the half-eviction strategy keeps cache within bounds
        even under sustained writes of unique keys.
        """
        engine = self._two_rule_engine("cap.md")

        from src.constants.routing import ROUTING_MATCH_CACHE_MAX_SIZE

        # Write 3x the max capacity worth of unique entries
        for i in range(ROUTING_MATCH_CACHE_MAX_SIZE * 3):
            await engine.match(make_msg(sender_id=f"cap-{i}", text=f"cap-{i}"))

        # Cache must never exceed max_size
        assert len(engine._match_cache) <= ROUTING_MATCH_CACHE_MAX_SIZE, (
            f"Cache size {len(engine._match_cache)} exceeds max "
            f"{ROUTING_MATCH_CACHE_MAX_SIZE}"
        )

        # Cache should be non-empty after writes
        assert len(engine._match_cache) > 0, "Cache should not be empty after writes"
