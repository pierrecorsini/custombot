...
# ── DB guarded write span tests ────────────────────────────────────────────


class TestDbGuardedWriteSpan:
    """Test db_guarded_write_span async context manager."""
    ...
    async def test_attributes_set_on_real_span(self):
        """Verify span attributes when OTel is available."""
        mock_span = MagicMock()
        ...
        with (
            patch.object(tracing_mod, "_OTEL_AVAILABLE", True),
            patch.object(tracing_mod, "get_tracer", return_value=mock_tracer),
        ):
            reset_tracer()
            async with db_guarded_write_span(
        ...
        assert call_args[0][0] == "db.guarded_write"
        attrs = call_args[1]["attributes"]
        assert attrs["custombot.db.operation"] == "upsert_chat"
        assert attrs["custombot.db.max_retries"] == 3
        assert attrs["custombot.db.retry_budget_total"] == 5.0


# ── DB retry attempt span tests ────────────────────────────────────────────


class TestDbRetryAttemptSpan:
    """Test db_retry_attempt_span sync context manager."""

    def test_yields_span_with_noop_tracer(self):
        with patch.object(tracing_mod, "_OTEL_AVAILABLE", False):
            reset_tracer()
            with db_retry_attempt_span(attempt=0) as span:
                assert isinstance(span, _NoOpSpan)
    ...
    def test_attributes_set_on_real_span(self):
        ...
        with (
            ...
            reset_tracer()
            with db_retry_attempt_span(attempt=2) as span:
                assert span is mock_span

        mock_tracer.start_as_current_span.assert_called_once()
        call_args = mock_tracer.start_as_current_span.call_args
        assert call_args[0][0] == "db.guarded_write.retry"
        attrs = call_args[1]["attributes"]
        assert attrs["custombot.db.retry.attempt"] == 2

    def test_caller_can_set_extra_attributes(self):
        """Verify callers can add delay/budget attributes on the span."""
        mock_span = MagicMock()
        mock_span.is_recording.return_value = True
        mock_span.__enter__ = MagicMock(return_value=mock_span)
        mock_span.__exit__ = MagicMock(return_value=False)
        ...
        with (
            ...
            reset_tracer()
            with db_retry_attempt_span(attempt=1) as span:
                span.set_attribute("custombot.db.retry.delay_seconds", 0.25)
                span.set_attribute("custombot.db.retry.budget_remaining", 1.5)

        mock_span.set_attribute.assert_any_call(
            "custombot.db.retry.delay_seconds", 0.25,
        )
        mock_span.set_attribute.assert_any_call(
            "custombot.db.retry.budget_remaining", 1.5,
        )
