...
class TestMessageQueueClose:
    ...
    async def test_persists_pending_on_close(self, tmp_path: Path):
        ...
        assert len(lines) >= 1
        data = decode_queue_line(lines[-1])
...
class TestMessageQueuePersistence:
    ...
    async def test_persist_writes_utf8(self, tmp_path: Path):
        ...
        raw.decode("utf-8")
        # Decode the line and verify the emoji roundtrips correctly
        lines = qfile.read_text(encoding="utf-8").strip().splitlines()
        data = decode_queue_line(lines[-1])
        assert data["text"] == "emoji 🎉 test"
...
# ═══════════════════════════════════════════════════════════════════════════════
# 13b. CRC32 persistence integrity checks
# ═══════════════════════════════════════════════════════════════════════════════


class TestCRC32Integrity:
    """Tests for CRC32 checksum guards on queue-file persistence."""

    def test_encode_record_produces_crc_prefix(self):
        """_encode_record emits lines in CRC32:base64(msgpack) format."""
        from src.message_queue_persistence import _encode_record

        data = {"message_id": "crc-1", "chat_id": "chat-crc", "text": "hello", "status": "pending"}
        line = _encode_record(data)

        parts = line.split(":", 1)
        assert len(parts) == 2, f"Expected CRC:payload format, got: {line[:40]}"
        crc_hex, b64_payload = parts
        assert len(crc_hex) == 8
        assert all(c in "0123456789abcdef" for c in crc_hex)

    def test_decode_line_verifies_crc32(self):
        """_decode_line successfully decodes CRC-guarded lines."""
        from src.message_queue_persistence import _encode_record

        data = {"message_id": "crc-2", "chat_id": "chat-crc", "text": "world", "status": "pending"}
        line = _encode_record(data)
        result = _decode_line(line, log_errors=False)

        assert result is not None
        assert result["message_id"] == "crc-2"
        assert result["text"] == "world"

    def test_decode_line_rejects_tampered_crc(self):
        """_decode_line returns None when CRC32 is corrupted."""
        from src.message_queue_persistence import _encode_record

        data = {"message_id": "crc-3", "chat_id": "chat-crc", "text": "tampered"}
        line = _encode_record(data)

        # Flip a bit in the CRC prefix
        tampered = "ffffffff" + line[8:]
        result = _decode_line(tampered, log_errors=False)

        assert result is None

    def test_decode_line_detects_truncated_payload(self):
        """_decode_line returns None when the base64 payload is truncated."""
        from src.message_queue_persistence import _encode_record

        data = {"message_id": "crc-4", "chat_id": "chat-crc", "text": "truncated msg"}
        line = _encode_record(data)

        # Truncate the base64 payload (keep original CRC, chop payload)
        crc_prefix = line[:9]  # "xxxxxxxx:"
        truncated = crc_prefix + line[9:20]  # Only first 11 chars of payload
        result = _decode_line(truncated, log_errors=False)

        assert result is None

    def test_decode_line_legacy_base64_still_works(self):
        """Legacy base64(msgpack) lines without CRC prefix still decode."""
        import base64
        from src.utils import msgpack_dumps

        data = {"message_id": "legacy-1", "chat_id": "chat-legacy", "text": "old format"}
        legacy_line = base64.b64encode(msgpack_dumps(data)).decode("ascii")

        result = _decode_line(legacy_line, log_errors=False)
        assert result is not None
        assert result["message_id"] == "legacy-1"

    def test_decode_line_json_fallback_still_works(self):
        """Plain JSON lines (no CRC, no base64) still decode."""
        result = _decode_line('{"message_id": "json-1", "chat_id": "c", "text": "t"}', log_errors=False)
        assert result is not None
        assert result["message_id"] == "json-1"

    def test_has_crc_prefix_detects_guarded_lines(self):
        """_has_crc_prefix correctly identifies CRC-guarded lines."""
        from src.message_queue_persistence import _encode_record, _has_crc_prefix

        guarded = _encode_record({"x": 1})
        assert _has_crc_prefix(guarded) is True

        assert _has_crc_prefix('{"plain": "json"}') is False
        assert _has_crc_prefix("") is False
        assert _has_crc_prefix("short") is False

    async def test_load_pending_tracks_checksum_mismatches(self, tmp_path: Path):
        """load_pending() records CRC mismatches in checksum_mismatches field."""
        from src.message_queue_persistence import _encode_record, QueuePersistence

        data_dir = tmp_path / "data"
        data_dir.mkdir()
        qfile = data_dir / "message_queue.jsonl"

        # Write one valid CRC-guarded line
        valid_data = make_queued(message_id="crc-ok-1").to_dict()
        valid_line = _encode_record(valid_data)

        # Write one line with a bad CRC
        tampered = "deadbeef" + valid_line[8:]

        qfile.write_text(valid_line + "\n" + tampered + "\n", encoding="utf-8")

        persistence = QueuePersistence(qfile)
        pending, result = persistence.load_pending()

        assert result is not None
        assert result.is_corrupted is True
        assert result.checksum_mismatches == [2]
        assert len(pending) == 1
        assert "crc-ok-1" in pending

    async def test_persist_messages_writes_crc_guarded_lines(self, tmp_path: Path):
        """persist_messages() produces CRC-guarded msgpack lines."""
        from src.message_queue_persistence import _has_crc_prefix, QueuePersistence

        data_dir = tmp_path / "data"
        data_dir.mkdir()
        qfile = data_dir / "message_queue.jsonl"

        msg = make_queued(message_id="persist-crc-1")
        persistence = QueuePersistence(qfile)
        persistence.persist_messages([msg])

        content = qfile.read_text(encoding="utf-8")
        lines = content.strip().splitlines()
        assert len(lines) == 1

        # Every line from persist_messages must have CRC prefix
        assert _has_crc_prefix(lines[0]) is True

        # And must be decodeable
        data = _decode_line(lines[0], log_errors=False)
        assert data is not None
        assert data["message_id"] == "persist-crc-1"

    async def test_validate_sync_detects_crc_mismatches(self, tmp_path: Path):
        """validate_sync() reports checksum_mismatches for corrupted CRC lines."""
        from src.message_queue_persistence import _encode_record, QueuePersistence
        ...
        valid_data = make_queued(message_id="v-crc-ok").to_dict()
        valid_line = _encode_record(valid_data)
        tampered = "00000000" + valid_line[8:]

        qfile.write_text(valid_line + "\n" + tampered + "\n", encoding="utf-8")

        persistence = QueuePersistence(qfile)
        result = persistence.validate_sync()

        assert result.is_corrupted is True
        assert result.checksum_mismatches == [2]
        assert result.valid_lines == 1

    async def test_roundtrip_survives_full_cycle(self, tmp_path: Path):
        """Full lifecycle: persist → load → persist again produces valid CRC-guarded data."""
        from src.message_queue_persistence import _has_crc_prefix, QueuePersistence
        ...
        msgs = [make_queued(message_id=f"rt-{i}", text=f"msg {i}") for i in range(5)]

        persistence = QueuePersistence(qfile)

        # First persist
        persistence.persist_messages(msgs)
        ...
        # Second persist (eager eviction path)
        persistence.persist_messages(list(pending.values()))

        # Verify all lines have CRC prefix
        content = qfile.read_text(encoding="utf-8")
        for line in content.strip().splitlines():
            assert _has_crc_prefix(line), f"Line missing CRC prefix: {line[:30]}"
