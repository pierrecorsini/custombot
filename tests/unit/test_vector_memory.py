...
class TestConnect:
    ...
    def test_read_connection_enables_wal_journal_mode(self, vm: VectorMemory):
        """Per-thread read connections should also use WAL mode."""
        read_conn = vm._get_read_connection()
        row = read_conn.execute("PRAGMA journal_mode").fetchone()
        assert row[0].lower() == "wal"

    def test_read_connection_enables_foreign_keys(self, vm: VectorMemory):
        """Per-thread read connections should apply DEFAULT_PRAGMAS including foreign_keys."""
        read_conn = vm._get_read_connection()
        row = read_conn.execute("PRAGMA foreign_keys").fetchone()
        assert row[0] == 1
...
# ─────────────────────────────────────────────────────────────────────────────
# Embedding Cache Eviction Under Pressure
# ─────────────────────────────────────────────────────────────────────────────


class TestEmbedCacheEvictionPressure:
    """Tests for embedding cache eviction under pressure.

    Verifies that inserting more entries than the cache max_size causes
    the oldest entries to be evicted (with production "half" eviction
    strategy), cache hit rate degrades as expected, and memory doesn't
    grow beyond the configured bounds.
    """

    @pytest.mark.asyncio
    async def test_oldest_entries_evicted_when_cache_exceeds_max(self, db_path: Path):
        """Insert 3× max_size unique texts and verify oldest entries are evicted.

        With eviction="half" (production default), the cache evicts the oldest
        half of entries when it reaches max_size. After inserting 30 unique
        entries into a size-10 cache, none of the first 10 texts should remain.
        """
        client = _make_mock_client()
        vm = VectorMemory(str(db_path), client, EMBEDDING_MODEL, EMBEDDING_DIM)
        vm.connect()
        cache_size = 10
        vm._embed_cache = BoundedOrderedDict(max_size=cache_size, eviction="half")

        texts = [f"pressure-text-{i}" for i in range(cache_size * 3)]

        # Act — embed all 30 texts (3× the cache capacity)
        for text in texts:
            await vm._embed(text)

        # Assert — cache never exceeds max
        assert len(vm._embed_cache) <= cache_size

        # First 10 texts should all be evicted (pushed out by later inserts)
        first_batch_keys = [_cache_key(t) for t in texts[:cache_size]]
        assert all(k not in vm._embed_cache for k in first_batch_keys), (
            "All oldest entries should be evicted after 3× pressure"
        )

        # Last 10 texts should all be present (most recently inserted)
        last_batch_keys = [_cache_key(t) for t in texts[-cache_size:]]
        assert all(k in vm._embed_cache for k in last_batch_keys), (
            "All most-recent entries should still be in cache"
        )

        vm.close()

    @pytest.mark.asyncio
    async def test_cache_size_never_exceeds_max_under_pressure(self, db_path: Path):
        """Rapidly inserting many entries should never let cache grow beyond max_size."""
        client = _make_mock_client()
        vm = VectorMemory(str(db_path), client, EMBEDDING_MODEL, EMBEDDING_DIM)
        vm.connect()
        cache_size = 20
        ...
        # Act — embed 200 unique texts (10× cache capacity)
        for i in range(200):
            await vm._embed(f"burst-text-{i}")
            # Assert — cache never exceeds max at any intermediate step
            assert len(vm._embed_cache) <= cache_size, (
                f"Cache grew to {len(vm._embed_cache)} after embedding text {i + 1} "
    ...
    @pytest.mark.asyncio
    async def test_hit_rate_degrades_for_evicted_entries(self, db_path: Path):
        """After eviction, re-requesting evicted texts triggers API calls (cache misses)."""
        call_count = 0
        client = _make_mock_client()
        original_create = client.embeddings.create
        ...
        # Arrange — embed 5 early texts (fills cache completely)
        early_texts = [f"early-{i}" for i in range(cache_size)]
        for text in early_texts:
            await vm._embed(text)
        assert call_count == cache_size

        # Insert more texts to trigger eviction of early entries
        for i in range(cache_size):
            await vm._embed(f"later-{i}")
        calls_after_later = call_count
        assert calls_after_later == cache_size * 2

        # Act — re-request all early texts
        misses = 0
        for text in early_texts:
            calls_before = call_count
            await vm._embed(text)
            if call_count > calls_before:
                misses += 1

        # Assert — at least some early texts should be cache misses
        assert misses > 0, (
            f"Expected cache misses for evicted entries, got 0 misses "
            f"out of {cache_size} early texts"
        )
...
class TestEmbeddingCacheEviction:
    """Verify embedding cache evicts entries under pressure."""

    @pytest.mark.asyncio
    async def test_cache_respects_max_size(self, db_path: Path):
        """Cache should not grow beyond its configured maximum."""
        client = _make_mock_client()
        vm = VectorMemory(str(db_path), client, EMBEDDING_MODEL, EMBEDDING_DIM)
        vm.connect()
        cache_size = 10
        ...
        # Add more entries than the cache max size — every intermediate
        # step must stay within bounds.
        for i in range(cache_size * 3):
            await vm._embed(f"overflow-text-{i}")
            assert len(vm._embed_cache) <= cache_size, (
                f"Cache size {len(vm._embed_cache)} exceeded max {cache_size} "
                f"after inserting entry {i + 1}"
            )

        vm.close()

    @pytest.mark.asyncio
    async def test_cache_evicts_oldest_entries(self, db_path: Path):
        """When cache is full, oldest entries are evicted first (LRU)."""
        client = _make_mock_client()
        vm = VectorMemory(str(db_path), client, EMBEDDING_MODEL, EMBEDDING_DIM)
        vm.connect()
        cache_size = 3
        vm._embed_cache = BoundedOrderedDict(max_size=cache_size, eviction="one")

        # Fill cache to capacity
        await vm._embed("oldest")
        await vm._embed("middle")
        await vm._embed("newest")
        assert len(vm._embed_cache) == 3

        # Adding one more should evict the oldest entry
        await vm._embed("overflow")
        assert len(vm._embed_cache) == 3

        # "oldest" was the first inserted — it should be evicted
        assert _cache_key("oldest") not in vm._embed_cache

        # "newest" and "overflow" should still be present
        assert _cache_key("newest") in vm._embed_cache
        assert _cache_key("overflow") in vm._embed_cache

        vm.close()

    @pytest.mark.asyncio
    async def test_cache_lookup_prevents_eviction(self, db_path: Path):
        ...
        # Fill cache: insertion order is [first, second, third]
        await vm._embed("first")
        await vm._embed("second")
        await vm._embed("third")

        # Access "first" again — LRU refresh moves it to the end:
        # internal order becomes [second, third, first]
        await vm._embed("first")

        # Adding a new entry evicts the current oldest ("second")
        await vm._embed("fourth")
        assert len(vm._embed_cache) == 3

        # "first" was NOT evicted because it was recently accessed
        assert _cache_key("first") in vm._embed_cache
        # "second" was evicted because it became the oldest after the refresh
        assert _cache_key("second") not in vm._embed_cache
        assert _cache_key("fourth") in vm._embed_cache

        vm.close()

    @pytest.mark.asyncio
    async def test_cache_survives_high_pressure(self, db_path: Path):
        """Adding many entries rapidly doesn't corrupt the cache."""
        client = _make_mock_client()
        vm = VectorMemory(str(db_path), client, EMBEDDING_MODEL, EMBEDDING_DIM)
        vm.connect()
        cache_size = 20
        ...
        # Add 2× max_size entries in rapid succession
        num_entries = cache_size * 2
        texts = [f"pressure-{i}" for i in range(num_entries)]
        for text in texts:
            await vm._embed(text)

        # Cache must not exceed its configured maximum
        assert len(vm._embed_cache) <= cache_size, (
            f"Cache size {len(vm._embed_cache)} exceeded max {cache_size}"
        )

        # Every present key must map to a valid embedding (no corruption)
        for key in list(vm._embed_cache.keys()):
            embedding = vm._embed_cache[key]
            assert isinstance(embedding, list)
            assert len(embedding) == EMBEDDING_DIM

        # The most recent entries should be present (not evicted)
        recent_keys = [_cache_key(t) for t in texts[-cache_size:]]
        present_recent = sum(1 for k in recent_keys if k in vm._embed_cache)
        assert present_recent > 0, "Expected at least some recent entries in cache"
