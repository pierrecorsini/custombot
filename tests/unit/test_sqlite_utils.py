...
class TestSqliteHelperRetry:
    ...
    def test_retries_transient_error_then_succeeds(self, helper: _ConcreteHelper) -> None:
        ...
        mock_db = MagicMock()
        mock_db.execute = MagicMock(side_effect=flaky_execute)
        helper._db = mock_db
    ...
    def test_non_transient_error_not_retried(self, helper: _ConcreteHelper) -> None:
        ...
        mock_db = MagicMock()
        mock_db.execute = MagicMock(side_effect=schema_error)
        helper._db = mock_db
    ...
    def test_commit_uses_guarded_path(self, helper: _ConcreteHelper) -> None:
        ...
        # First do an execute with real db
        with helper._lock:
            helper._db.execute("INSERT INTO t (id, v) VALUES (?, ?)", (1, "test"))

        mock_db = MagicMock()
        mock_db.commit = MagicMock(side_effect=flaky_commit)
        helper._db = mock_db
...
class TestGetSqlConnectionFactory:
    """Verify get_sqlite_connection accepts a custom connection_factory."""

    def test_custom_factory_is_used(self, tmp_path: Path) -> None:
        """The provided factory is called instead of sqlite3.connect."""
        ...
        get_sqlite_connection(db_path, connection_factory=tracking_factory)

        assert len(calls) == 1
        assert str(db_path) in calls[0]

    def test_custom_factory_connection_has_pragmas(self, tmp_path: Path) -> None:
        """PRAGMAs are applied to connections from custom factory."""
        from src.db.sqlite_utils import get_sqlite_connection
    ...
    def test_default_factory_when_none(self, tmp_path: Path) -> None:
        """Passing None uses the default sqlite3.connect factory."""
        from src.db.sqlite_utils import get_sqlite_connection

        conn = get_sqlite_connection(tmp_path / "default.db", connection_factory=None)
        assert isinstance(conn, sqlite3.Connection)
        conn.close()


class TestSqliteHelperConnectionFactory:
    """Verify SqliteHelper._open_connection accepts a custom connection_factory."""

    def test_fallback_uses_custom_factory(self, tmp_path: Path) -> None:
        """When no pool is set, _open_connection uses the custom factory."""
        calls: list[str] = []
        ...
        h = _ConcreteHelper(tmp_path / "fallback.db")
        h._open_connection(connection_factory=tracking_factory)

        assert len(calls) == 1
        assert str(tmp_path / "fallback.db") in calls[0]
        h.close()

    def test_pool_factory_takes_precedence(self, tmp_path: Path) -> None:
        """When pool is set, connection_factory parameter is ignored (pool owns creation)."""
        ...
        try:
            ...
            h = _ConcreteHelper(tmp_path / "pool.db")
            # connection_factory should be ignored — pool creates the connection
            h._open_connection(connection_factory=tracking_factory)

            assert len(calls) == 0  # Custom factory NOT called
            assert h._db is not None
            h.close()
        finally:
            SqliteHelper.set_pool(None)  # type: ignore[arg-type]
            pool.close_all()
