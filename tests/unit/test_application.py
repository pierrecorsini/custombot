...
class TestOnMessageConcurrency:
    ...
    async def test_n_plus_one_message_waits_for_semaphore_slot(
        self, test_config: Config
    ) -> None:
        """The (N+1)th message is blocked until one of N in-flight messages completes.

        Dispatch N+1 messages with max_concurrent_messages=N.  Verify that only
        the first N enter the pipeline, the (N+1)th is queued, and then starts
        as soon as a slot is freed.
        """
        n = 2
        test_config.max_concurrent_messages = n
        app = Application(test_config)

        mock_shutdown = MagicMock()
        mock_shutdown.accepting_messages = True

        # Per-message events for fine-grained control.
        started: list[asyncio.Event] = [asyncio.Event() for _ in range(n + 1)]
        release: list[asyncio.Event] = [asyncio.Event() for _ in range(n + 1)]
        ...
        # Dispatch N+1 messages concurrently.
        tasks = [
            asyncio.create_task(app._on_message(_make_msg(message_id=f"msg-{i}")))
            for i in range(n + 1)
        ]

        # Wait for the first N messages to acquire slots and enter the pipeline.
        for i in range(n):
            await asyncio.wait_for(started[i].wait(), timeout=1.0)

        # The (N+1)th message must NOT have started — it's queued at the semaphore.
        assert not started[n].is_set(), (
            f"Message {n} should be waiting for a semaphore slot, not executing"
        )

        # Free one slot by completing the first message.
        release[0].set()

        # The (N+1)th message should now acquire the freed slot and start.
        await asyncio.wait_for(started[n].wait(), timeout=1.0)
        assert started[n].is_set()

        # Clean up — release remaining messages.
        for ev in release:
            ev.set()
        await asyncio.gather(*tasks)
...
class TestSwapConfigAtomicity:
    ...
    def test_swap_resizes_semaphore(self, tmp_path: "Path") -> None:
        """_swap_config replaces the semaphore when max_concurrent_messages changes."""
        cfg_a = self._config_a(tmp_path)
        app = Application(cfg_a)
        old_semaphore = app._message_semaphore
        assert old_semaphore._value == cfg_a.max_concurrent_messages

        # Build config B with a different max_concurrent_messages
        cfg_b = self._config_b(tmp_path)
        cfg_b.max_concurrent_messages = cfg_a.max_concurrent_messages + 5

        app._swap_config(cfg_b)

        assert app._message_semaphore is not old_semaphore
        assert app._message_semaphore._value == cfg_b.max_concurrent_messages

    def test_swap_preserves_semaphore_when_unchanged(self, tmp_path: "Path") -> None:
        """_swap_config keeps the same semaphore when max_concurrent_messages is unchanged."""
        cfg_a = self._config_a(tmp_path)
        app = Application(cfg_a)
        old_semaphore = app._message_semaphore

        cfg_b = self._config_b(tmp_path)
        cfg_b.max_concurrent_messages = cfg_a.max_concurrent_messages

        app._swap_config(cfg_b)

        assert app._message_semaphore is old_semaphore
