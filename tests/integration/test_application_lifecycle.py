...
class TestConfigHotReload:
    ...
    @pytest.mark.asyncio
    async def test_max_concurrent_messages_hot_reload_resizes_semaphore(
        self, tmp_path: Path
    ) -> None:
        """Hot-reloading ``max_concurrent_messages`` from 1→3 allows 3 concurrent messages.

        Verifies the full hot-reload pipeline: ConfigWatcher detects the file
        change → ConfigChangeApplier classifies it as safe →
        ``on_config_swap`` calls ``Application._swap_config`` → the semaphore
        is replaced → new arrivals see the updated concurrency cap.

        In-flight messages on the old semaphore are NOT affected; only NEW
        arrivals after the hot-reload see the resized semaphore.
        """
        from src.app import AppComponents, AppPhase

        config_path = tmp_path / "config.json"

        # ── Arrange: start with max_concurrent_messages=1 ──
        initial_config = Config(
            llm=LLMConfig(
                model="gpt-4o",
                base_url="https://api.openai.com/v1",
        ...
        app = Application(initial_config)
        assert app._message_semaphore._value == 1

        # Wire up a mock pipeline that tracks in-flight concurrency.
        in_flight = 0
        max_in_flight = 0
        ...
        async def _track_execute(ctx: Any) -> None:
            ...
            # Extract message index from message_id ("msg-0", "msg-1", "msg-2")
            idx = int(ctx.msg.message_id.split("-")[1])
            in_flight += 1
        ...
        # Set up ConfigChangeApplier wired with on_config_swap
        mock_bot = MagicMock()
        mock_bot._cfg = BotConfig(
            max_tool_iterations=10,
            memory_max_history=100,
        ...
        applier = ConfigChangeApplier(
            ...
            shutdown_mgr=mock_shutdown,
            reconfigure_logging=reconfigure_logging,
            on_config_swap=app._swap_config,
        )
        ...
        try:
            ...
            # Sanity: semaphore is still 1
            assert app._message_semaphore._value == 1

            # ── Act: write updated config with max_concurrent_messages=3 ──
            updated_config = Config(
                llm=LLMConfig(
                    model="gpt-4o",
                    base_url="https://api.openai.com/v1",
            ...
            # Wait for the watcher to detect, load, and apply
            await asyncio.sleep(0.5)

            # Verify the semaphore was resized
            assert app._message_semaphore._value == 3, (
                f"Expected semaphore value=3 after hot-reload, "
                f"got {app._message_semaphore._value}"
            )

            # ── Assert: 3 messages can now run concurrently ──
            tasks = [
                asyncio.create_task(
                    app._on_message(_make_msg(message_id=f"msg-{i}"))
                )
                for i in range(3)
            ]

            # Wait for all 3 to start — they should ALL start since
            # the semaphore now allows 3 concurrent acquires.
            for i in range(3):
                await asyncio.wait_for(started[i].wait(), timeout=1.0)

            # All 3 are in-flight simultaneously
            assert max_in_flight == 3, (
                f"Expected 3 concurrent messages after hot-reload, "
                f"got max_in_flight={max_in_flight}"
            )

            # Clean up — release all messages
            for i in range(3):
                release[i].set()
...
# ─────────────────────────────────────────────────────────────────────────────
# Test: Graceful shutdown during active ReAct loop
# ─────────────────────────────────────────────────────────────────────────────


class TestGracefulShutdownDuringReActLoop:
    """Integration test for graceful shutdown while a message is mid-ReAct-loop.
    ...
    Exercises the critical production path:
        ...
        _on_message(msg)
          ...
          → pipeline.execute(ctx)
            → OperationTrackerMiddleware  (enter_operation)
            → PreflightMiddleware
            → HandleMessageMiddleware
              → bot.handle_message(msg)
                → _handle_message_inner(msg)
                  → _chat_locks.acquire(chat_id)   # per-chat lock held
                  → message_queue.enqueue(msg)     # PENDING
                  → asyncio.wait_for(_process(...), timeout=per_chat_timeout)
                    → _react_loop(...)              # HANGS (mocked)
          ← request_shutdown() called mid-processing
          ← per_chat_timeout expires → asyncio.TimeoutError
                  → _chat_locks release (async with exits)
                  → message_queue.complete(msg)     # PENDING → COMPLETED
                  → return None
            → exit_operation (in-flight op deregistered)
          → semaphore release

    Verifies:
        (a) per-chat lock is released after shutdown cancels processing
        (b) message queue entry is completed (not left PENDING)
        (c) no orphaned asyncio tasks remain
        (d) GracefulShutdown in-flight count returns to 0
    """

    @pytest.mark.asyncio
    async def test_shutdown_mid_react_loop_releases_lock_completes_queue(
        self, tmp_path: Path
    ) -> None:
        """
        When shutdown is requested while a message is mid-ReAct-loop, the
        per-chat timeout expires, the lock is released, the queue entry is
        completed, and no orphaned tasks remain.
        """
        from src.app import AppComponents, AppPhase
        from src.bot import Bot
        from src.bot._bot import BotDeps
        ...
        bot_config = BotConfig(
            ...
            system_prompt_prefix="",
            stream_response=False,
            per_chat_timeout=per_chat_timeout,
        )

        mock_db = AsyncMock()
        mock_db.get_generation = MagicMock(return_value=0)
        mock_db.upsert_chat_and_save_message = AsyncMock()
        mock_llm = AsyncMock()
        mock_memory = AsyncMock()
        mock_memory.ensure_workspace = MagicMock(return_value=tmp_path / "ws")
        mock_memory.read_memory = AsyncMock(return_value=None)
        mock_skills = MagicMock()
        mock_skills.all = MagicMock(return_value=[])
        mock_skills.tool_definitions = []

        # Real message queue backed by tmp_path
        from src.message_queue import MessageQueue

        data_dir = tmp_path / "queue_data"
        data_dir.mkdir()
        real_queue = MessageQueue(str(data_dir))
        await real_queue.connect()

        chat_locks = LRULockCache(max_size=100)
        ...
        # Mock _process to simulate a long-running ReAct loop that
        # exceeds per_chat_timeout (0.10s)
        async def _hanging_process(*_args: Any, **_kwargs: Any) -> str:
            await asyncio.sleep(10)

        mock_channel = _make_mock_channel()
        real_shutdown = GracefulShutdown(timeout=5.0)
        ...
        msg = _make_msg(
            message_id="msg-shutdown-react-1",
            chat_id="chat-shutdown-react",
        )

        # ── Act: start message processing + request shutdown mid-loop ──
        with (
            patch.object(bot, "_process", side_effect=_hanging_process),
            patch("src.bot._bot.log"),
            patch("src.core.message_pipeline.log_message_flow"),
        ):
            # Launch _on_message in the background — it will enter the
            # ReAct loop and hang until per_chat_timeout expires.
            processing_task = asyncio.create_task(app._on_message(msg))

            # Give the pipeline a moment to enter _handle_message_inner
            # and acquire the per-chat lock + enqueue the message.
            await asyncio.sleep(0.03)

            # ── Request shutdown while ReAct loop is active ──
            real_shutdown.request_shutdown()

            # Wait for the processing to finish (per_chat_timeout will
            # cancel it within 0.10s of the _process start)
            await processing_task

        # ── Assert (a): per-chat lock is released ──
        assert chat_locks.active_count == 0, (
            f"Expected 0 active locks after shutdown, got {chat_locks.active_count}"
        )

        # ── Assert (b): message queue entry is completed ──
        # The message should NOT be in _pending (it was completed in the
        # finally block of _handle_message_inner).
        assert "msg-shutdown-react-1" not in real_queue._pending, (
            "Message queue entry was left PENDING after shutdown — "
            "expected it to be completed in finally cleanup"
        )

        # Close the message queue (stops FlushManager background task)
        # before checking for orphaned tasks.
        await real_queue.close()

        # ── Assert (c): no orphaned asyncio tasks ──
        ...
        assert len(orphaned) == 0, (
            f"Found {len(orphaned)} orphaned tasks after shutdown: "
            f"{[t.get_name() for t in orphaned]}"
        )

        # ── Assert (d): GracefulShutdown in-flight count is 0 ──
        assert real_shutdown._in_flight_count == 0, (
            f"Expected 0 in-flight operations after shutdown, "
            f"got {real_shutdown._in_flight_count}"
        )
