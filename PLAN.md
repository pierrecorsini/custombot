...
- [ ] Replace bare `except Exception: pass` patterns with structured error handling — audit all 15+ instances of silent exception swallowing (especially in `bot.py` event emission, `memory.py` `_track_cache_event`, `vector_memory.py`) and replace with explicit error codes or re-raise where appropriate
