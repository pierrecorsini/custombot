<!-- Context: project/concepts/architecture-overview | Priority: high | Version: 2.8 | Updated: 2026-05-07 -->
...
- **Structured DI**: `BotDeps` dataclass replaces 15-param Bot constructor; `ShutdownContext` dataclass for teardown; `ComponentRegistry` for lightweight fail-fast DI; `RateLimiter`, `ToolExecutor`, `ContextAssembler` now constructed in `builder.py` and injected via `BotDeps`
- **Structured logging**: `StructuredContextFilter` auto-injects correlation_id, chat_id, app_phase, session_id into every LogRecord via ContextVar
...
- **Incremental chats persistence**: `Database._save_chats()` uses dirty-set tracking + append-only changelog (O(dirty) per flush) with periodic compaction back to full `chats.json` snapshot (threshold: 200 entries)
- **Retry budget recovery**: `Database._retry_budget_spent` resets to 0 on `_guarded_write` success, preventing transient filesystem degradation from permanently disabling retries
...
main.py (Click CLI)
  └─ Application (AppPhase state machine, bounded semaphore, error categorization)
       ...
       │         → typing → error_handler → handle_message
         ...
         │    ├─ context_building.py (TurnContext from routing + instruction + memory)
         │    ├─ crash_recovery.py (stale message recovery)
         │    ├─ response_delivery.py (post-ReAct: filter, dedup, persist, emit)
        ...
        ├─ EventBus (cross-component pub/sub, backpressure semaphore, 9+ events)
        ├─ HealthCheckRegistry (centralized discoverable health checks)
        ├─ FlushManager (swap-buffers non-blocking write management)
...
| Response Delivery | `src/bot/response_delivery.py` | Post-ReAct: filter, dedup, persist, emit |
